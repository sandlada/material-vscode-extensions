...
接線真相以 `package.json` → `contributes` 為準；`README.md` 的支援狀況表只是狀態記錄，改 `schema/` 增減 family 時記得同步更新（只列實際有的 family）。

## 指令（皆在根目錄跑）
- `npm run compile`：`packages/material-web-snippets` 的 `tsc -p ./`，`src/` → `out/`（該包 `vscode:prepublish` 會自動跑）。該包 `tsconfig.json` 的 `exclude` 只有 `node_modules`、`.vscode-test`、`out`；根 `tsconfig.tools.json` 只管 `schema`＋`scripts`，工具鏈不會污染擴展構建。
- `npm run generate`：從 `schema/` 寫出兩包的 `snippets/*.code-snippets` 與 `custom-data/html.html-data.json`（另有全元件 `md-components` 索引）；新 family 在 `schema/` 加檔註冊後跑它即可。
...
- 打包／發佈必須帶 `--no-dependencies`（各包 `npm run package` 已內含）：npm workspaces 下 `vsce` 的 `npm list` 會回傳 repo 根，導致整個 repo（含 `.git`、隔壁包）被掃進 vsix；兩包都沒有 runtime dependencies，加此旗無副作用。發佈前用 `vsce ls --no-dependencies` 核對清單。
...
- README 中英雙語（英文為主、中文緊隨）；支援狀況表只列 `schema/` 實際有的 family，不列上游沒有的元件（如 Snackbar、Tooltip）。
- README 的數量口徑（HTML 條數、attr 變體數、CSS token 條數、tags / attrs 數）以落地檔案實測為準，改 `schema/` 後重算，不要沿用舊數字。
